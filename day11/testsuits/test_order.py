...
@allure.story("测试模块_order")             # 功能块，具有相同feature或story的用例将规整到相同模块下,执行时可用于筛选
@allure.issue("BUG号：345")                 # 问题表识，关联标识已有的问题，可为一个url链接地址
@allure.testcase("用例名：目录查询类")
@pytest.mark.parametrize("_,testcase,method,uri,header,body,exception",data,ids=[a[1] for a in data])
def test_main_order(_,testcase,method,uri,header,body,exception):
